    ...
    <div className="p-8 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button
          ...
          className="bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-semibold gap-2"
        >
                ...
                return (
                  <tr key={r.id} className="border-b border-slate-800/60 hover:bg-slate-800/40 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className="px-2 py-0.5 rounded text-xs text-purple-300 bg-purple-500/10 capitalize">
                        {r.tradeType}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`px-2 py-1 rounded-lg text-xs font-medium ${STATUS_COLOR[r.status] ?? "text-slate-400 bg-slate-800"}`}>
                        {STATUS_LABEL[r.status] ?? r.status}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      {pnl !== null ? (
                        <span className={`flex items-center gap-1 text-sm font-semibold ${pos ? "text-emerald-400" : "text-red-400"}`}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-1">
                        ...
                        <button
                          onClick={() => {
                            if (confirm(`Delete ${r.nseSymbol}?`)) deleteFn({ id: r.id });
                          }}
...
function PnlModal({
  ...
  rec: Recommendation;
  onSave: (d: { exitPrice: number; pnlPercent: number; pnlAbsolute: number; status: "target_hit" | "stop_loss_hit" | "partial_profit" | "closed" }) => void;
  isPending: boolean;
  onClose: () => void;
}) {
